...
import {
  searchUser,
  getAuthorizedUsers,
} from '../../store/actions/PrivateDiscussionAction';
...
const PrivateDiscussionModal = (props) => {
  ...
  const followers = useSelector(
    (state) => state.PrivateDiscussionReducer.users,
  );
  const authorized = useSelector(
    (state) => state.PrivateDiscussionReducer.authorized,
  );
  const authorizedId = useSelector(
    (state) => state.PrivateDiscussionReducer.authorizedId,
  );
  const noFollowers = useSelector(
    (state) => state.PrivateDiscussionReducer.noUsers,
  );
  const userCount = useSelector(
    (state) => state.PrivateDiscussionReducer.userCount,
  );

  const [selectedUser, setSelectedUser] = useState(
    fromDiscussionScreen ? authorizedId : selectedFollowers,
  );
  ...
  const searchUserName = (searchInput, clearSearch) => {
    ...
    if (fromDiscussionScreen) {
      dispatch(
        getAuthorizedUsers(discussionId, clearSearch ? '' : searchInput),
      );
    } else {
  ...
  const FollowerList = ({
    ...
    return (
      <TouchableOpacity
        ...
        onPress={() =>
          isSelected(userId) ? deselectUser(userId) : selectUser(userId)
        }
        style={styles.followerDataContainerStyle}>
        <View
          style={
  ...
  const nextPage = () => {
    ...
    if (fromDiscussionScreen) {
      dispatch(
        getAuthorizedUsers(discussionId, searchKeyword, false, currentPage + 1),
      );
    } else {
