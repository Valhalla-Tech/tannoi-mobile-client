...
const EmailConfirmationModal = (props) => {
  ...
  const emailVerified = async () => {
    try {
      setIsLoading(true);
      let emailActivationRequest = await axios.get(
        `${BaseUrl}/users/activation?token=${getCode()}&device=${Platform.OS}`,
      );
      if (emailActivationRequest.data.msg === 'Email Activated') {
        ...
        await AsyncStorage.setItem(
          'access_token',
          emailActivationRequest.data.access_token,
        );
        await AsyncStorage.setItem(
          'refresh_token',
          emailActivationRequest.data.refresh_token,
        );
  ...
  const resendCode = async () => {
    try {
      let resendCodeRequest = await axios.get(
        `${BaseUrl}/users/generate/${userId}`,
      );
    ...
    return (
      <>
        <TouchableWithoutFeedback onPress={() => Keyboard.dismiss()}>
          <View style={styles.confirmationModalStyle}>
            <CloseButton closeFunction={closeEmailConfirmationModal} />
            <Text style={styles.confirmationEmailModalTitleStyle}>
              Confirm your email
            </Text>
            <Text style={styles.confirmationEmailModalInstructionStyle}>
              Enter the 4-digit code tannOi just sent to {emailAddress}
            </Text>
            {codeValidation && <ErrorMessage message="Wrong code" />}
            {InputBox()}
            <View style={styles.sendAgainEmailContainerStyle}>
              <Text style={styles.sendAgainEmailTextStyle}>
                Didn't get an email?{' '}
              </Text>
              <TouchableOpacity onPress={resendCode}>
                <Text style={styles.sendAgainEmailButtonTextStyle}>
                  Send again
                </Text>
              </TouchableOpacity>
            </View>
