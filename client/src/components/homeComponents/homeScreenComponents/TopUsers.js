  ...
  const TopUserContent = () => {
    return (
      <View>
        ...
        {!topUserData && <LoadingSpinner loadingSpinnerForComponent={true} />}
        <View style={styles.listStyle}>
          <FlatList
            ...
            renderItem={(itemData) => (
              <TouchableOpacity
                style={styles.topUsersCardConntainerStyle}
                onPress={() => {
                ...
                {itemData.item.name === null ? (
                  <Text style={styles.topUsersNameStyle}> </Text>
                ) : itemData.item.name.length > 14 ? (
                  <Text
                    style={
                      styles.topUsersNameStyle
                    }>{`${itemData.item.name.substr(0, 11)}...`}</Text>
                ) : (
