...
const NotificationSettingsScreen = ({ navigation }) => {
  ...
  const getUserNotificationStatus = async () => {
    try {
      ...
      if (getUserNotificationStatusRequest.data) {
        setEmailNotificationStatus(getUserNotificationStatusRequest.data.emailNotification.All);
      }
    } catch (error) {
      console.log(error);
    }
  ...
  const emailSwitch = async () => {
    try {
      ...
      let emailSwitchRequest = await axios({
        method: 'post',
        url: emailNotificationStatus ? `${BaseUrl}/users/delete-notification` : `${BaseUrl}/users/add-notification`,
        headers: {
          'token': access_token
        },
        data: {
  ...
  const NotificationSettingsSwitch = () => {
    return (
      <View style={styles.notificationSettingsSwitchStyle}>
        <Text style={styles.notificationSwithchTextStyle}>Send me an email</Text>
        <Switch
  ...
  const NotificationSettingsCardContent = () => {
    return (
      <View>
        {
          emailNotificationStatus === '' ? <LoadingSpinner loadingSpinnerForComponent={true} /> : (
            <>
